...
namespace Hw11.Services.ExpressionValidator;

public class ExpressionValidator : IExpressionValidator
{
    private readonly HashSet<char> _validOperations = new() { '+', '-', '*', '/' };
    ...
    public void Validate(string? expression)
    {
        ...
        if (_validOperations.Contains(expr[0]))
            throw new Exception(MathErrorMessager.StartingWithOperation);
        if (_validOperations.Contains(expr[^1]))
    ...
    private bool ContainsTwoOperationsInARow(string expression, out char? op1, out char? op2)
    {
        ...
        foreach (var ch in expression)
        {
            if (_validOperations.Contains(ch))
                if (stack.Count == 0)
                    stack.Push(ch);
                else
                {
    ...
    private string FindOperationBeforeClosingParenthesis(string expression)
    {
        for(var i = 0; i < expression.Length - 1; i++)
            if (_validOperations.Contains(expression[i]) && expression[i + 1] is ')')
    ...
    {
        for(var i = 0; i < expression.Length - 1; i++)
            if (expression[i] is '(' && _validOperations.Contains(expression[i+1]))
    ...
    private int IndexOfUnknownCharacter(string expression)
    {
        ...
        for (var i = 0; i < expression.Length; i++)
            if (!char.IsDigit(expression[i]) 
                && !_validOperations.Contains(expression[i]) 
