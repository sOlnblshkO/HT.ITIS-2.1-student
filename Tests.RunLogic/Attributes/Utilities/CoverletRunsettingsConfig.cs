...
{
    internal static void UpdateCodeCoverletRunsettings(int homeworkNumber,
        string coverletRunsettingsPath = ".github/coverlet.runsettings",
        string studentExcludesPath = ".github/coverlet.runsettings.studentExcludes.txt")
    {
        ...
        var match = Regex.Match(coverletRunsettingsContent, @"<ExcludeByFile>(.*?)<\/ExcludeByFile>",
        ...
        var studentCustomExcludesContent = File.Exists(studentExcludesFullPath)
            ? File.ReadAllText(studentExcludesFullPath)
            : string.Empty;

        // Build the updated ExcludeByFile content based on the homework number
        var updatedExcludeByFileContentStr =
            GenerateUpdatedExcludesContent(studentCustomExcludesContent, homeworkNumber);
        if (excludeByFileContent == updatedExcludeByFileContentStr) return;

        // Construct the updated coverlet.runsettings file content
        var newCoverletRunsettingsContent = UpdateFileContent(coverletRunsettingsContent, excludeByFileTag,
            excludeByFileContent, updatedExcludeByFileContentStr);
        File.WriteAllText(coverletRunsettingsFullPath, newCoverletRunsettingsContent);
    }

    private static string UpdateFileContent(string fileFullContent, string olgTag, string oldTagContent,
    ...
    {
        var updatedExcludeByFileContent =
            new StringBuilder("**/Tests.RunLogic/**,**/*.cshtml,**/Hw*/Program.cs,**/Hw*/Program.fs");
